...
const buildDefaultUsers = () => {
  ...
  const rdEmails = new Set(["jrazo@arguspsm.com","kcole@arguspsm.com","rjurado@arguspsm.com","dlay@arguspsm.com"]);
  const rdFacilities = {};
  FACILITIES_DATA.forEach(f => {
    const key = f.rdEmail.toLowerCase();
    if (!rdFacilities[key]) rdFacilities[key] = [];
    if (!rdFacilities[key].includes(f.code)) rdFacilities[key].push(f.code);
  });
  const rdUsers = [
    { id: "rd_jrazo", email: "jrazo@arguspsm.com", name: "Joe Razo", role: "rd", password: "argusops2026", facilities: rdFacilities["jrazo@arguspsm.com"] || [] },
    { id: "rd_kcole", email: "Kcole@arguspsm.com", name: "Kyla Cole", role: "rd", password: "argusops2026", facilities: rdFacilities["kcole@arguspsm.com"] || [] },
...
export default function App() {
  ...
  const UnitRow = ({ unit }) => {
    ...
    const canEdit = ["manager", "admin", "executive", "rd"].includes(currentUser.role);
            ...
            <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 10 }}>
                  {["manager","admin","executive","rd"].includes(currentUser.role) && <Btn small onClick={() => setModal({ type: "add_unit" })}>+ Add Unit</Btn>}
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                {["manager","admin","executive","rd"].includes(currentUser.role) && <Btn onClick={() => setModal({ type: "add_unit" })}>+ Add Unit</Btn>}
              </div>
            </div>
