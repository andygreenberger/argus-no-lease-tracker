...
const getStatus = (unit) => {
  ...
  const facilityStatus = FACILITY_OVERRIDES[unit.facility_code] || "active";
  if (facilityStatus === "tracking_only") return "TRACKING_COMPLETE";
...
const STATUS_CONFIG = {
  ...
  TRACKING_COMPLETE: { label: "Process Complete (Tracking Only)", color: "#065f46", bg: "#d1fae5", dot: "#10b981" },
...
const exportToExcel = (units, facilities) => {
  ...
  const buckets = [
    ...
    { label: "7 - Process Complete (Tracking Only)", statuses: ["TRACKING_COMPLETE"] },
  ];

  const headers = [
    "Stage", "Facility Code", "Facility Name", "State", "Facility Status", "Unit Number",
  ...
  buckets.forEach(bucket => {
    ...
    bucketUnits.forEach(u => {
      ...
      rows.push([
        ...
        u.facility_name || "",
        fac.state || "",
        FACILITY_OVERRIDES[u.facility_code] === "tracking_only" ? "L2L Tracking Only" :
          FACILITY_OVERRIDES[u.facility_code] === "offboarded" ? "Offboarded" : "Active",
        u.unit_number || "",
...
export default function App() {
  ...
  const loadFacilityOverrides = async () => {
    try {
      const data = await sb("facility_overrides?select=code,status");
      const map = {};
      data.forEach(d => { map[d.code] = d.status; });
      FACILITY_OVERRIDES = map;
      setFacilityOverrides(map);
    } catch (e) {
      console.warn("Could not load facility overrides:", e);
    }
  };

  const archiveUnit = async (unit, reason, actorName) => {
    const loggedBy = actorName.trim() || currentUser.email;
    try {
      await updateUnit(unit.id, {
        archived: true,
        archived_at: new Date().toISOString(),
        archived_by: loggedBy,
        archive_reason: reason,
      });
      showToast(`Unit ${unit.unit_number} archived.`);
      setModal(null);
    } catch (e) {
  ...
  const setFacilityStatus = async (code, status) => {
    setSaving(true);
    try {
      await sb(`facility_overrides?code=eq.${code}`, { method: "DELETE", prefer: "return=minimal" });
      if (status !== "active") {
        await sb("facility_overrides", {
          method: "POST",
          body: JSON.stringify({ code, status, updated_by: currentUser.email }),
        });
      }
      const newMap = { ...FACILITY_OVERRIDES };
      if (status === "active") delete newMap[code];
      else newMap[code] = status;
      FACILITY_OVERRIDES = newMap;
      setFacilityOverrides(newMap);
      showToast("Facility status updated.");
    } catch (e) {
      showToast("Error updating facility status.", "error");
    }
    setSaving(false);
  };

  useEffect(() => { loadUnits(); loadFacilityOverrides(); }, []);
  ...
  const myFacilities = currentUser
    ? (currentUser.role === "admin" || currentUser.role === "executive")
      ? FACILITIES_DATA
      : FACILITIES_DATA.filter(f =>
          (currentUser.facilities || []).some(fc => fc.toLowerCase() === f.code.toLowerCase()) &&
          (facilityOverrides[f.code] !== "offboarded")
        )
    : [];

  const myFacilityCodes = new Set(myFacilities.map(f => f.code));
  const myUnits = units.filter(u => myFacilityCodes.has(u.facility_code) && !u.archived);
  ...
  const stats = {
    ...
    trackingComplete: myUnits.filter(u => getStatus(u) === "TRACKING_COMPLETE").length,
  };
  ...
  const renderModal = () => {
    ...
    if (modal.type === "archive_unit") return <ArchiveModal unit={modal.unit} onSave={archiveUnit} onClose={() => setModal(null)} saving={saving} currentUser={currentUser} />;
    if (modal.type === "manage_facilities") return <FacilityManagementModal facilities={FACILITIES_DATA} overrides={facilityOverrides} onSetStatus={setFacilityStatus} saving={saving} onClose={() => setModal(null)} />;
    if (modal.type === "view_unit") return <ViewUnitModal unit={modal.unit} fac={getFacility(modal.unit.facility_code)} currentUser={currentUser} onArchive={(unit) => setModal({ type: "archive_unit", unit })} onClose={() => setModal(null)} />;
    return null;
  };
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16, marginBottom: 28 }}>
              {[
                ...
                { label: "Complete (Tracking Only)", value: stats.trackingComplete, color: "#065f46", bg: "#d1fae5", statusKey: "TRACKING_COMPLETE" },
              ].map(s => (
        ...
        {view === "admin" && currentUser.role === "admin" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", padding: 20 }}>
                <h3 style={{ margin: "0 0 8px", fontSize: 15, fontWeight: 700 }}>Facility Status</h3>
                <p style={{ margin: "0 0 12px", fontSize: 13, color: "#6b7280" }}>Mark facilities as Active, L2L Tracking Only, or Fully Offboarded. Admin only.</p>
                <Btn variant="secondary" onClick={() => setModal({ type: "manage_facilities" })}>🏢 Manage Facilities</Btn>
              </div>
              <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", padding: 20 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", padding: 20 }}>
                ...
                <p style={{ margin: "4px 0", fontSize: 13 }}>Tracking only complete: <strong>{units.filter(u => getStatus(u) === "TRACKING_COMPLETE").length}</strong></p>
                <p style={{ margin: "4px 0", fontSize: 13 }}>Archived (hidden): <strong>{units.filter(u => u.archived).length}</strong></p>
                <p style={{ margin: "4px 0", fontSize: 13 }}>Facilities with custom status: <strong>{Object.keys(facilityOverrides).length}</strong></p>
...
function ViewUnitModal({ unit, fac, currentUser, onArchive, onClose }) {
  const status = getStatus(unit);
  return (
    <Modal title={`Unit ${unit.unit_number} — ${unit.facility_name}`} onClose={onClose}>
      ...
      {currentUser?.role === "admin" && (
        <div style={{ borderTop: "1px solid #f3f4f6", paddingTop: 12, marginTop: 8, display: "flex", justifyContent: "flex-end" }}>
          <Btn variant="secondary" small onClick={() => onArchive(unit)}>Archive Unit</Btn>
        </div>
      )}
    </Modal>
  );
...
// ── ARCHIVE MODAL ─────────────────────────────────────────────────────────────
function ArchiveModal({ unit, onSave, onClose, saving, currentUser }) {
  const [reason, setReason] = useState("Lease Signed");
  const [otherText, setOtherText] = useState("");
  const [actorName, setActorName] = useState(currentUser.name || currentUser.email);
  const [confirmed, setConfirmed] = useState(false);
  const finalReason = reason === "Other" ? (otherText.trim() || "Other") : reason;
  return (
    <Modal title={`Archive Unit — ${unit.unit_number}`} onClose={onClose}>
      <p style={{ margin: "0 0 16px", fontSize: 14, color: "#6b7280" }}>{unit.facility_name}</p>
      <div style={{ background: "#fef3c7", border: "1px solid #fde68a", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
        <p style={{ margin: 0, fontSize: 13, color: "#92400e" }}>⚠ Archived units are permanently hidden from all users and all reports. This action cannot be undone from within the app.</p>
      </div>
      <Field label="Reason for archiving">
        <select value={reason} onChange={e => setReason(e.target.value)} style={{ width: "100%", padding: "9px 12px", border: "1px solid #d1d5db", borderRadius: 7, fontSize: 14 }}>
          <option>Lease Signed</option>
          <option>Tenant Vacated</option>
          <option>Error / Added by Mistake</option>
          <option>Other</option>
        </select>
      </Field>
      {reason === "Other" && (
        <Field label="Describe reason">
          <Input value={otherText} onChange={e => setOtherText(e.target.value)} placeholder="Brief description..." />
        </Field>
      )}
      ...
      <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
        <label style={{ display: "flex", gap: 10, alignItems: "flex-start", cursor: "pointer" }}>
          <input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} style={{ marginTop: 2 }} />
          <span style={{ fontSize: 13, color: "#374151" }}>I confirm this unit should be archived and understand it will no longer appear in the tracker or any reports.</span>
        </label>
      </div>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        <Btn variant="secondary" onClick={onClose}>Cancel</Btn>
        <Btn variant="danger" onClick={() => onSave(unit, finalReason, actorName)} disabled={!confirmed || saving}>Archive Unit</Btn>
      </div>
    </Modal>
  );
}
...
function FacilityManagementModal({ facilities, overrides, onSetStatus, saving, onClose }) {
  const [search, setSearch] = useState("");
  const [filterStatus, setFilterStatus] = useState("ALL");

  const getFacStatus = (code) => overrides[code] || "active";

  const statusLabel = { active: "Active", tracking_only: "L2L Tracking Only", offboarded: "Fully Offboarded" };
  const statusColors = {
    active: { color: "#064e3b", bg: "#ecfdf5" },
    tracking_only: { color: "#1e3a5f", bg: "#eff6ff" },
    offboarded: { color: "#7c2d12", bg: "#fef2f2" },
  };

  const filtered = facilities.filter(f => {
    const fs = getFacStatus(f.code);
  ...
  const nonActiveCodes = Object.keys(overrides);

  return (
    <Modal title="Facility Status Management" onClose={onClose}>
      <div style={{ background: "#fef3c7", border: "1px solid #fde68a", borderRadius: 6, padding: "8px 12px", marginBottom: 16, fontSize: 12, color: "#92400e" }}>
        ⚠ Changes save to the database immediately and affect all users at once. Admin only.
      </div>
      <div style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 6, padding: "8px 12px", marginBottom: 14, fontSize: 12, color: "#0c4a6e" }}>
        <strong>Active</strong> — normal. &nbsp;|&nbsp; <strong>L2L Tracking Only</strong> — process ends at DM sign-off, no Late2Lien release required. &nbsp;|&nbsp; <strong>Fully Offboarded</strong> — hidden from all non-admin users.
      </div>
      <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search facility name or code..." style={{ flex: 1 }} />
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: "9px 12px", border: "1px solid #d1d5db", borderRadius: 7, fontSize: 13 }}>
          <option value="ALL">All ({facilities.length})</option>
          <option value="active">Active</option>
          <option value="tracking_only">Tracking Only ({nonActiveCodes.filter(c => overrides[c] === "tracking_only").length})</option>
          <option value="offboarded">Offboarded ({nonActiveCodes.filter(c => overrides[c] === "offboarded").length})</option>
        </select>
      </div>
      <div style={{ maxHeight: 400, overflowY: "auto" }}>
        {filtered.map(f => {
          const cur = getFacStatus(f.code);
          const { color, bg } = statusColors[cur] || statusColors.active;
          return (
            <div key={f.code} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #f3f4f6" }}>
              <div style={{ minWidth: 0, flex: 1, marginRight: 12 }}>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#111827", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{f.name}</p>
                <p style={{ margin: "2px 0 0", fontSize: 11, color: "#6b7280" }}>{f.code} · {f.state} · {f.dmName || "—"}</p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 10, color, background: bg, whiteSpace: "nowrap" }}>{statusLabel[cur]}</span>
                <select
                  value={cur}
                  onChange={e => onSetStatus(f.code, e.target.value)}
                  disabled={saving}
                  style={{ padding: "5px 8px", border: "1px solid #d1d5db", borderRadius: 6, fontSize: 12, cursor: "pointer" }}
                >
                  <option value="active">Active</option>
                  <option value="tracking_only">L2L Tracking Only</option>
                  <option value="offboarded">Fully Offboarded</option>
                </select>
              </div>
            </div>
          );
        })}
        {filtered.length === 0 && <p style={{ color: "#9ca3af", fontSize: 14, textAlign: "center", padding: "20px 0" }}>No facilities match.</p>}
      </div>
      <p style={{ margin: "10px 0 0", fontSize: 12, color: "#9ca3af" }}>Showing {filtered.length} of {facilities.length} facilities</p>
    </Modal>
  );
}
